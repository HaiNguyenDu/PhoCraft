...
import com.example.phocraft.databinding.FragmentSelfiesBinding
...
class SelfiesFragment : Fragment() {
    private val binding by lazy { FragmentSelfiesBinding.inflate(layoutInflater) }
    ...
    private fun observeLiveData() {
        viewModel.selfieImages.observe(viewLifecycleOwner, Observer { listImage ->
            imageAdapter.submitList(listImage)
        })
        viewModel.loadImages(ImageCategory.SELFIES)
    }
}